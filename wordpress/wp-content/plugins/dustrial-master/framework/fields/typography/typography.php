 ...
 *
 */
class DUSTRIALFramework_Option_typography extends DUSTRIALFramework_Options {
  ...
  public function output() {
    ...
    $default_variants = apply_filters( 'dustrial_websafe_fonts_variants', array(
      'regular',
      'italic',
      '700',
      '700italic',
    ...
    $websafe_fonts = apply_filters( 'dustrial_websafe_fonts', array(
      'Arial',
      'Arial Black',
      'Comic Sans MS',
      'Impact',
    ...
    $google_json   = dustrial_get_google_fonts();
    ...
    if( is_object( $google_json ) ) {
      ...
      echo '<label class="dustrial-typography-family">';
      echo '<select name="'. $this->element_name( '[family]' ) .'" class="'. $is_chosen . $chosen_rtl .'dustrial-typo-family" data-atts="family"'. $this->element_attributes() .'>';

      do_action( 'dustrial_typography_family', $family_value, $this );

      echo '<optgroup label="'. esc_html__( 'Web Safe Fonts', 'dustrial-framework' ) .'">';
      ...
      echo '<optgroup label="'. esc_html__( 'Google Fonts', 'dustrial-framework' ) .'">';
      ...
      if( ! empty( $is_variant ) ) {
        ...
        echo '<label class="dustrial-typography-variant">';
        echo '<select name="'. $this->element_name( '[variant]' ) .'" class="'. $is_chosen . $chosen_rtl .'dustrial-typo-variant" data-atts="variant">';
      ...
      echo '<input type="text" name="'. $this->element_name( '[font]' ) .'" class="dustrial-typo-font hidden" data-atts="font" value="'. $value['font'] .'" />';

    } else {

      echo esc_html__( 'Error! Can not load json file.', 'dustrial-framework' );
