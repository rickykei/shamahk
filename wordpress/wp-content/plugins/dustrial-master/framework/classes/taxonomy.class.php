 ...
 *
 */
class DUSTRIALFramework_Taxonomy extends DUSTRIALFramework_Abstract{
  ...
  public function __construct( $options ) {
    ...
    $this->options = apply_filters( 'dustrial_taxonomy_options', $options );
  ...
  public static function instance( $options = array() ) {
    if ( is_null( self::$instance ) && DUSTRIAL_ACTIVE_TAXONOMY ) {
  ...
  public function add_taxonomy_fields() {
    ...
    foreach ( $this->options as $option ) {
      ...
      $get_taxonomy = dustrial_get_var( 'taxonomy' );
  ...
  public function render_taxonomy_form_fields( $term ) {
    ...
    $dustrial_errors = get_transient( 'dustrial-taxonomy-transient' );

    wp_nonce_field( 'dustrial-taxonomy', 'dustrial-taxonomy-nonce' );

    echo '<div class="dustrial-framework dustrial-taxonomy dustrial-taxonomy-'. $classname .'-fields">';
      ...
      foreach( $this->options as $option ) {
        ...
        if( $taxonomy == $option['taxonomy'] ) {
          ...
          foreach ( $option['fields'] as $field ) {
            ...
            echo dustrial_add_element( $field, $elem_value, $option['id'] );
  ...
  public function save_taxonomy( $term_id ) {
    ...
    if ( wp_verify_nonce( dustrial_get_var( 'dustrial-taxonomy-nonce' ), 'dustrial-taxonomy' ) ) {

      $errors = array();
      $taxonomy = dustrial_get_var( 'taxonomy' );
      ...
      foreach ( $this->options as $request_value ) {
        ...
        if( $taxonomy == $request_value['taxonomy'] ) {
            ...
            foreach( $request_value['fields'] as $field ) {
              ...
              if( isset( $field['type'] ) && isset( $field['id'] ) ) {
                ...
                if( has_filter( 'dustrial_sanitize_'. $sanitize_type ) ) {
                  $request[$field['id']] = apply_filters( 'dustrial_sanitize_' . $sanitize_type, $field_value, $field, $request_value['fields'] );
                }

                // validate options
                if ( isset( $field['validate'] ) && has_filter( 'dustrial_validate_'. $field['validate'] ) ) {

                  $validate = apply_filters( 'dustrial_validate_' . $field['validate'], $field_value, $field, $request_value['fields'] );
          ...
          $request = apply_filters( 'dustrial_save_taxonomy', $request, $request_key, $term_id );
      ...
      set_transient( 'dustrial-taxonomy-transient', $errors, 10 );
