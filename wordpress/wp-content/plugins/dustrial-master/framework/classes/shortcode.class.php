 ...
 *
 */
class DUSTRIALFramework_Shortcode_Manager extends DUSTRIALFramework_Abstract{
  ...
  public function __construct( $options ) {
    ...
    $this->options = apply_filters( 'dustrial_shortcode_options', $options );
    $this->exclude_post_types = apply_filters( 'dustrial_shortcode_exclude', $this->exclude_post_types );
    ...
    if( ! empty( $this->options ) ) {
      ...
      $this->addAction( 'wp_ajax_dustrial-get-shortcode', 'shortcode_generator', 99 );
  ...
  public static function instance( $options = array() ){
    if ( is_null( self::$instance ) && DUSTRIAL_ACTIVE_SHORTCODE ) {
  ...
  public function media_shortcode_button( $editor_id ) {
    ...
    if( ! in_array( $post_type, $this->exclude_post_types ) ) {
      echo '<a href="#" class="button button-primary dustrial-shortcode" data-editor-id="'. $editor_id .'">'. esc_html__( 'Add Shortcode', 'dustrial-framework' ) .'</a>';
    }
  ...
  ?>
    <div id="dustrial-shortcode-dialog" class="dustrial-dialog" title="<?php esc_html_e( 'Add Shortcode', 'dustrial-framework' ); ?>">
      <div class="dustrial-dialog-header">
        <select class="<?php echo ( is_rtl() ) ? 'chosen-rtl ' : ''; ?>dustrial-dialog-select" data-placeholder="<?php esc_html_e( 'Select a shortcode', 'dustrial-framework' ); ?>">
          <option value=""></option>
          <?php
      ...
      <div class="dustrial-dialog-load"></div>
      <div class="dustrial-insert-button hidden">
        <a href="#" class="button button-primary dustrial-dialog-insert"><?php esc_html_e( 'Insert Shortcode', 'dustrial-framework' ); ?></a>
      </div>
    </div>
  <?php
  }
  ...
  public function shortcode_generator() {
    ...
    $request = dustrial_get_var( 'shortcode' );
    ...
    if( isset( $shortcode['fields'] ) ) {
      ...
      foreach ( $shortcode['fields'] as $key => $field ) {
        ...
        echo dustrial_add_element( $field, $field_default, 'shortcode' );
    ...
    if( isset( $shortcode['clone_fields'] ) ) {
      ...
      echo '<div class="dustrial-shortcode-clone" data-clone-id="'. $clone_id .'">';
      echo '<a href="#" class="dustrial-remove-clone"><i class="fa fa-trash"></i></a>';
      ...
      echo '<div class="dustrial-clone-button"><a id="shortcode-clone-button" class="button" href="#"><i class="fa fa-plus-circle"></i> '. $shortcode['clone_title'] .'</a></div>';
